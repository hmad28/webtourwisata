...
export default function Home() {
  return <>
    ...
    <section id="experiences" className="section shell home-experiences"><div className="mobile-section-head"><h2 className="section-title">Popular experiences</h2><Link href="/tour-packages">See all <ArrowRight size={18} /></Link></div><div className="experience-grid">{experiences.map(([name, image, Icon]) => <article key={name} className="experience-card"><div><Image src={image} alt={name} fill sizes="240px" /></div><span className="experience-icon"><Icon /></span><h3>{name}</h3></article>)}</div></section>

    <section className="section shell home-tours"><h2 className="section-title"><span className="desktop-tour-title">Most Loved Tour Packages</span><span className="mobile-tour-title">Trips worth waking up early for</span></h2><p className="section-lede">Flexible starting points for your Lombok adventure. Final pricing and availability are confirmed directly.</p><div className="tour-grid">{tours.slice(0,3).map(tour => <TourCard tour={tour} key={tour.slug} />)}</div><div className="center-action"><Link href="/tour-packages" className="text-link">View all tour packages <ArrowRight size={18} /></Link></div></section>

    <section className="section shell home-destinations"><h2 className="section-title">Explore Lombok</h2><div className="destination-mosaic">{destinations.slice(0,4).map((d, i) => <Link href={`/destinations/${d.slug}`} className={`destination-tile tile-${i + 1}`} key={d.slug}><Image src={d.image} alt={d.name} fill sizes="(max-width: 720px) 50vw, 25vw" /><div className="tile-shade" /><div><h3>{d.name}</h3><p>{d.tagline}</p></div></Link>)}</div></section>
