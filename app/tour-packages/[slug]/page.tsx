...
export default async function TourDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <section className="detail-hero"><Image src={tour.image} alt={tour.title} fill priority sizes="100vw" /><div className="hero-shade" /><div><span>{tour.category}</span><h1>{tour.title}</h1><p>{tour.shortDescription}</p></div></section>
